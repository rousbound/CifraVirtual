...
class Main:
    def __init__(self):
        ...
        self.SONGS_FOLDER = "Music/"
        self.COMPOSITIONS_FOLDER = "MyMusic/"
        self.state = ""
    ...
    def connect(self):
       ...
       self.ui.musicList.itemClicked.connect\
        (lambda: self.openItem\
         (self.ui.musicList,self.ui.musicTextDisplay))
       ...
       self.ui.musicAddEntryButton.clicked.connect\
        (lambda: self.newListItem\
          (self.ui.musicLineEntry,self.ui.musicList,self.ui.musicTextDisplay))

       self.ui.actionDelete_Song.triggered.connect\
        (lambda: self.deleteListItem\
          (self.ui.musicList))

       self.ui.musicToggleButton.clicked.connect\
        (lambda: self.audioHandler.toggleMusic\
          (self,self.ui.musicList))

      #Audio playback buttons 
       self.ui.musicReplayButton.clicked.connect\
        (lambda: self.audioHandler.replayMusic\
          (self.ui.musicList))

       self.ui.musicRewindButton.clicked.connect\
        (lambda: self.audioHandler.changeTimePos\
          (-5000))
    ...
    def getTxtFilePathFromList(self,listWidget):
      return self.state + listWidget.currentItem().text() + ".txt"
    ...
    def save(self,listWidget,display):
        if self.state != "": 
          ...
          def overwrite(path):
              ...
              file.write(str(text))
              file.close()
          filePath = self.getTxtFilePathFromList(listWidget)
          overwrite(filePath)
        else:
    ...
    def openItem(self,listWidget,display):
    ...
    def OpenWindow(self,definer):
      ...
      if definer == "songs":
        self.state = self.SONGS_FOLDER
        self.loadListFiles(self.state,self.ui.musicList)
      if definer == "compositions":
        self.state = self.COMPOSITIONS_FOLDER
        self.loadListFiles(self.state,self.ui.musicList)
